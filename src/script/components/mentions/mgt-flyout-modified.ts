...
import { styles } from './mgt-flyout-modified-css';
 ...
 * @extends {LitElement}
 */
@customElement('mgt-flyout-modified')
export class MgtFlyout extends LitElement {
  ...
  protected render() {
    ...
    return html`
      <div class=${classMap(flyoutClasses)}>
        <div class="anchor">${anchorTemplate}</div>
        ${flyoutTemplate}
      </div>
    `;
  }
  ...
  protected renderFlyout(): TemplateResult {
    return html` <slot name="flyout"></slot> `;
  }
  ...
  private updateFlyout() {
    ...
    if (flyout && anchor) {
      ...
      if (flyoutRect.width + 2 * this._edgePadding > windowRect.width) {
        ...
        anchorRect.left + flyoutRect.width + this._edgePadding >
        windowRect.width
      ) {
      ...
      if (flyoutRect.height + 2 * this._edgePadding > windowRect.height) {
        ...
        if (windowRect.height - anchorRect.top + flyoutRect.height < 0) {
          ...
          bottom = Math.max(
            windowRect.height - anchorRect.top,
            this._edgePadding
          );
        }
      } else {
    ...
    if (flyout) {
      ...
      if (!e.composedPath) {
        ...
        while (currentElem) {
          ...
          if (
            currentElem === flyout ||
            (e.type === 'pointerdown' && currentElem === this)
          ) {
            return;
          }
        }
        ...
        if (
          path.includes(flyout) ||
          (e.type === 'pointerdown' && path.includes(this))
        ) {
          return;
        }
      }
